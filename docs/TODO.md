...
- [ ] **P0-2** `requirements.txt` 작성
  ...
  - tavily-python
  - python-dotenv
- [ ] **P0-3** `.env.example` 작성 (OPENAI_API_KEY, TAVILY_API_KEY)
- [ ] **P0-4** `pip install -r requirements.txt` 실행 및 환경 검증
- [ ] **P0-5** Git 초기화 및 `.gitignore` 설정 (`.env`, `data/*.pdf`, `outputs/` 제외)

---

## Phase 1. 데이터 준비 (RAG 문서)

- [ ] **P1-1** PDF 문서 수집 (총 100페이지 이내 준수)
  - LGES 사업보고서 (최신)
  - CATL Annual Report (최신)
  - IEA Global EV Outlook 2024/2025
  - 국내 배터리 산업 보고서 (KEIT, KIET 등)
- [ ] **P1-2** 총 페이지 수 확인 및 100p 초과 시 우선순위 기준으로 문서 정제
- [ ] **P1-3** `data/` 디렉터리에 PDF 배치

---

## Phase 2. 문서 로더 및 벡터 DB 구축

- [ ] **P2-1** `agents/document_loader.py` 구현
  - PyMuPDF로 PDF 파싱
...
- [ ] **P2-4** FAISS 인덱스 생성 및 로컬 저장 (`vectorstore/`)
- [ ] **P2-5** 벡터 DB 로드 유틸리티 함수 작성
- [ ] **P2-6** 단위 테스트: 검색 쿼리로 관련 청크가 올바르게 반환되는지 확인

---

## Phase 3. 공통 유틸리티

- [ ] **P3-1** `agents/llm_config.py` — LLM 클라이언트 초기화 (GPT-4o-mini)
- [ ] **P3-2** `agents/web_search.py` — Tavily 검색 래퍼 구현
  - **확증 편향 방지**: 쿼리 1개 입력 시 긍정/비판/중립 3개 쿼리 자동 생성
  - 3방향 검색 결과를 병렬 실행 후 통합 반환
- [ ] **P3-3** `agents/state.py` — `BatteryAnalysisState` TypedDict 정의
- [ ] **P3-4** `prompts/` 디렉터리에 각 Agent별 프롬프트 템플릿 파일 작성
  - `prompts/rag_agent.txt`
  - `prompts/market_research.txt`
  - `prompts/lges_analysis.txt`
  - `prompts/catl_analysis.txt`
  - `prompts/comparison.txt`
  - `prompts/report_generator.txt`

---

## Phase 4. RAG Agent 구현 (Agentic RAG)

- [ ] **P4-1** `agents/rag_agent.py` 기본 구조 작성
- [ ] **P4-2** `retrieve` 노드 구현 — FAISS에서 관련 문서 검색 (top-k=5)
- [ ] **P4-3** `grade_documents` 노드 구현 — 검색 결과 관련성 평가 (LLM 기반)
- [ ] **P4-4** `rewrite_query` 노드 구현 — 관련 문서 부족 시 쿼리 재구성
- [ ] **P4-5** Agentic RAG 루프 조건 엣지 설정 (충분 → generate / 부족 → rewrite, 최대 3회)
- [ ] **P4-6** `generate` 노드 구현 — 검색 결과 기반 답변 생성
- [ ] **P4-7** 참고 출처(파일명, 페이지) 자동 수집 및 `references` 상태에 저장

---

## Phase 5. 분석 Agent 구현

- [ ] **P5-1** `agents/market_research_agent.py` 구현
  - RAG Agent 호출 (시장 배경 관련 문서 검색)
  - Web Search Agent 호출 (확증 편향 방지 적용)
  - 결과를 `market_background` 상태에 저장
- [ ] **P5-2** `agents/lges_analysis_agent.py` 구현
  - RAG + Web Search로 LGES 전략 데이터 수집
  - LGES 전략 분석 + SWOT(S/W 내부) 초안 작성
  - 결과를 `lges_strategy`, `lges_swot` 상태에 저장
- [ ] **P5-3** `agents/catl_analysis_agent.py` 구현
  - RAG + Web Search로 CATL 전략 데이터 수집
  - CATL 전략 분석 + SWOT(S/W 내부) 초안 작성
  - 결과를 `catl_strategy`, `catl_swot` 상태에 저장
- [ ] **P5-4** `agents/comparison_agent.py` 구현
...
- [ ] **P8-1** 전체 파이프라인 End-to-End 실행 테스트
- [ ] **P8-2** RAG 검색 품질 확인 (관련 없는 청크 비율 검토)
- [ ] **P8-3** 확증 편향 방지 동작 확인 (3방향 쿼리 결과 비교)
- [ ] **P8-4** 보고서 내용 완결성 검토 (목차 모든 섹션 포함 여부)
- [ ] **P8-5** REFERENCE 형식 검증 (기관보고서 / 학술논문 / 웹페이지 형식 준수)
